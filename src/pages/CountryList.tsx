...
const CountryList: React.FC = () => {
  ...
  const { countries, searched, loading, error } = useSelector(
  ...
  useEffect(() => {
    if (countries.length > 0) {
      const uniqueTimeZones = extractTimezones(countries);
      const uniqueRegions = extractRegions(countries);
  ...
  useEffect(() => {
    setVisibleCountries(countries.slice(0, currentPage * countriesPerPage));
